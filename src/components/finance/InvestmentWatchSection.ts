...
import { openInvestmentModal } from "./FinanceModals";
...
export class InvestmentWatchSection {
  ...
  render(container: HTMLElement): void {
    ...
    this.store.getInvestmentWatchItems().forEach((item) => {
      ...
      body.createDiv({ cls: "cow-meta-line" }).createSpan({ text: `${item.code} · ${item.type} · ¥${item.price} · ${item.changePercent}%${item.note ? ` · ${item.note}` : ""}` });
